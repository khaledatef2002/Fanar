    ...
    function instructor_application(){
        ...
        if($user->num_rows() > 0){
            ...
            if ($previous_data == 0) {
                ...
                $data['mother_language'] = $this->input->post('mother_lang');
                $data['other_language'] = implode(',', $this->input->post('other_lang'));
                

                $data['gender'] = $this->input->post('gender');
                $data['job_type'] = $this->input->post('apply_as');
                $data['message'] = $this->input->post('message');

                if(in_array('other', $this->input->post('subjects')))
                {
                    $subjects_array = $this->input->post('subjects');
                    $subjects_array[count($subjects_array) - 1] = $this->input->post('other-subjects');
                    $data['subjects'] = implode(',', $subjects_array);
                }
                else
                {
                    $data['subjects'] = implode(',', $this->input->post('subjects'));
                }
                $data['current_job'] = $this->input->post('other_jobs') ?? -1;
                $data['experince'] = $this->input->post('experince') ?? -1;

                $data['how_to_conduct']  = $this->input->post('how_to_conduct');
                $data['school_level'] = implode(',', $this->input->post('prefered_school'));
                $data['experince_years'] = $this->input->post('experince-years');
                $data['affiliate'] = $this->input->post('reference');
