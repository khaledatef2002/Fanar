...
class Crud_model extends CI_Model
{
    ...
    public function get_category($param1 = "")
    {
        if ($param1 != "") {
            $this->db->where('id', $param1);
        }
    ...
    public function get_bundles($param1 = "")
    {
        if ($param1 != "") {
            $this->db->where('id', $param1);
        }
    ...
    public function get_students($param1 = "")
    {
        if ($param1 != "") {
            $this->db->where('id', $param1);
        }
    ...
    public function get_coupon_category($param1 = "")
    {
        if ($param1 != "") {
            $this->db->where('coupon_id', $param1);
        }
        return $this->db->get('coupon_category');
    }

    public function get_coupon_sub_category($param1 = "")
    {
        if ($param1 != "") {
            $this->db->where('coupon_id', $param1);
        }
        return $this->db->get('coupon_sub_category');
    }

    public function get_coupon_bundle($param1 = "")
    {
        if ($param1 != "") {
            $this->db->where('coupon_id', $param1);
        }
        return $this->db->get('coupon_bundle');
    }

    public function get_coupon_student($param1 = "")
    {
        if ($param1 != "") {
            $this->db->where('coupon_id', $param1);
        }
        return $this->db->get('coupon_student');
    }
   ...
   // Category Sub Category Image Delete
    ...
    public function course_purchase($user_id, $method, $amount_paid, $param1 = "", $param2 = "")
    {
        ...
        foreach ($purchased_courses as $purchased_course) {
            ...
            if ($applied_coupon) {
                ...
                $discount = ($data['amount'] * $coupon_details['discount']) / 100;
    ...
    {
        if (isset($_POST['code']) && isset($_POST['discount_type']) && in_array($_POST['discount_type'], ['fixed', 'percent']) && !empty($_POST['code']) && isset($_POST['discount']) && !empty($_POST['discount']) && isset($_POST['expiry_date']) && !empty($_POST['expiry_date'])) {
            $data['code'] = $this->input->post('code');
            $data['discount'] = $this->input->post('discount') > 0 ? $this->input->post('discount') : 0;
            $data['min'] = filter_var($this->input->post('min') ,FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
            $data['expiry_date'] = strtotime($this->input->post('expiry_date'));
            $data['created_at'] = strtotime(date('D, d-M-Y'));
            $data['discount_type'] = $this->input->post('discount_type');

            $data['categories'] = $this->input->post('category_condition') ? 1 : 0;
            $data['sub_categories'] = $this->input->post('sub_category_condition') ? 1 : 0;
            $data['bundles'] = $this->input->post('bundle_condition') ? 1 : 0;
            $data['students'] = $this->input->post('student_condition') ? 1 : 0;
            ...
            if ($availability) {
                ...
                {
                    foreach($_POST['condition_category'] ?? [] as $id)
                    {
                        $condition_item = [];
                        $condition_item['category_id'] = $id;
                        $condition_item['coupon_id'] = $coupon_id;

                        $category = $this->db->get_where('category', array('id' => $id));

                        if($category->num_rows() > 0)
                        {
                            $this->db->insert('coupon_category', $condition_item);
                        }
                    }
                }

                if($data['sub_categories'] ?? 0 == 1)
                {
                    foreach($_POST['condition_sub_category'] ?? [] as $id)
                    {
                        $condition_item = [];
                        $condition_item['sub_category_id'] = $id;
                        $condition_item['coupon_id'] = $coupon_id;

                        $sub_category = $this->db->get_where('category', array('id' => $id));

                        if($sub_category->num_rows() > 0)
                        {
                            $this->db->insert('coupon_sub_category', $condition_item);
                        }
                    }
                }
                ...
                if($data['bundles'] ?? 0 == 1)
                {
                    foreach($_POST['condition_bundle'] ?? [] as $id)
                    {
                        $condition_item = [];
                        $condition_item['bundle_id'] = $id;
                        $condition_item['coupon_id'] = $coupon_id;

                        $bundle = $this->db->get_where('course_bundle', array('id' => $id));

                        if($bundle->num_rows() > 0)
                        {
                            $this->db->insert('coupon_bundle', $condition_item);
                        }
                    }
                }

                if($data['students'] ?? 0 == 1)
                {
                    foreach($_POST['condition_student'] ?? [] as $id)
                    {
                        $condition_item = [];
                        $condition_item['student_id'] = $id;
                        $condition_item['coupon_id'] = $coupon_id;

                        $bundle = $this->db->get_where('users', array('id' => $id, 'role_id' => 2, 'is_instructor' => 0));

                        if($bundle->num_rows() > 0)
                        {
                            $this->db->insert('coupon_student', $condition_item);
                        }
                    }
                }
    ...
    public function edit_coupon($coupon_id)
    {
        if (isset($_POST['code']) && isset($_POST['discount_type']) && in_array($_POST['discount_type'], ['fixed', 'percent']) && !empty($_POST['code']) && isset($_POST['discount']) && !empty($_POST['discount']) && isset($_POST['expiry_date']) && !empty($_POST['expiry_date'])) {
            ...
            $this->db->where('coupon_id',$coupon_id);
            $this->db->delete('coupon_sub_category');
            
            $this->db->where('coupon_id', $coupon_id);
            $this->db->delete('coupon_bundle');
            
            $this->db->where('coupon_id', $coupon_id);
            $this->db->delete('coupon_student');
            ...
            if ($availability) {
                ...
                if($data['categories'] ?? 0 == 1)
                {
                    foreach($_POST['condition_category']['item_id'] ?? [] as $key => $value)
                    {
                        $id = $_POST['condition_category']['id'][$key];
                        $category_id = $value;

                        $condition_item = [];
                        $condition_item['category_id'] = $category_id;
                        $condition_item['coupon_id'] = $coupon_id;

                        if($id != 'empty')
                        {
                            $validty = $this->db->get_where('coupon_category', array('id' => $id))->num_rows();

                            if($validty > 0)
                                continue;
                            else   
                                $condition_item['id'] = $id;
                        }


                        $category = $this->db->get_where('category', array('id' => $category_id));
                ...
                if($data['sub_categories'] ?? 0 == 1)
                {
                    foreach($_POST['condition_sub_category']['item_id'] ?? [] as $key => $value)
                    {
                        $id = $_POST['condition_sub_category']['id'][$key];
                        $sub_category_id = $value;

                        $condition_item = [];
                        $condition_item['sub_category_id'] = $sub_category_id;
                        $condition_item['coupon_id'] = $coupon_id;

                        if($id != 'empty')
                        {
                            $validty = $this->db->get_where('coupon_sub_category', array('id' => $id))->num_rows();
                        ...
                        $sub_category = $this->db->get_where('category', array('id' => $sub_category_id));
                ...
                if($data['bundles'] ?? 0 == 1)
                {
                    foreach($_POST['condition_bundle']['item_id'] ?? [] as $key => $value)
                    {
                        $id = $_POST['condition_bundle']['id'][$key];
                        $bundle_id = $value;

                        $condition_item = [];
                        $condition_item['bundle_id'] = $bundle_id;
                        $condition_item['coupon_id'] = $coupon_id;

                        if($id != 'empty')
                        {
                            $validty = $this->db->get_where('coupon_bundle', array('id' => $id))->num_rows();
                        ...
                        $bundle = $this->db->get_where('course_bundle', array('id' => $bundle_id));
                ...
                if($data['students'] ?? 0 == 1)
                {
                    foreach($_POST['condition_student']['item_id'] ?? [] as $key => $value)
                    {
                        $id = $_POST['condition_student']['id'][$key];
                        $student_id = $value;

                        $condition_item = [];
                        $condition_item['student_id'] = $student_id;
                        $condition_item['coupon_id'] = $coupon_id;

                        if($id != 'empty')
                        {
                            $validty = $this->db->get_where('coupon_student', array('id' => $id))->num_rows();
                        ...
                        $bundle = $this->db->get_where('users', array('id' => $student_id, 'role_id' => 2, 'is_instructor' => 0));
    ...
    public function get_discounted_price_after_applying_coupon($coupon_code)
    {
        ...
        $discounted_price = $this->coupon_model->calc_cart_discount($coupon_details, $this->session->userdata('cart_items'));
