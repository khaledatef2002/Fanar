...
class Tutor_booking extends CI_Controller
{
    ...
    public function list_of_tuitions_after_filter($uri_segement = 0)
    {
        ...
        foreach($_GET as $key => $value){
            ...
            if(is_array($value) || preg_match('/"/', strtolower($value)) >= 1 && strpos(strtolower($value),"script") >= 1){
                $this->session->set_flashdata('error_message', site_phrase('such_script_searches_are_not_allowed').'!');
                redirect(site_url('tutors'), 'refresh');
            }
    ...
    function success_booking_payment($payment_method = "")
    {
        ...
        if ($model_name != null) {
            ...
            $response = $this->$model_name->$payment_check_function($payment_method, 'tutor_booking');
        }
