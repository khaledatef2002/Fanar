...
class Course_bundles extends CI_Controller {
    ...
    public function apply_coupon()
    {
        ...
        $page_data['bundle_details'] = ["price" => $this->input->post('price'), "id" => $this->session->userdata('bundle_id')];
        $response['html'] = [
            'elem' => '.ammount',
            'content' => $this->load->view('frontend/' . get_frontend_settings('theme') . '/bundle_amount_section.php', $page_data, true)
        ];
    ...
    function buy($bundle_id = ""){
        ...
        $coupon_code = $this->session->userdata('bundle_applied_coupon');

        if(!empty($coupon_code))
        {
            $appliable = $this->coupon_model->is_coupon_conditions_appliable_bundle($coupon_code, $bundle_details['id']);
            if(!$appliable) return false;
            $coupon_discounted_price = $this->coupon_model->calc_discount_bundle($coupon_code, $bundle_details['id']);
        }
        ...
        $items[] = $item_details;
        $total_payable_amount += $bundle_details['price'] - $coupon_discounted_price;
        ...
        if($total_payable_amount == 0)
        {
            ...
            $dataFree['user_id'] = $this->session->userdata('user_id');
            $dataFree['bundle_creator_id'] = $payment_details['items'][0]['creator_id'];
            $dataFree['bundle_id'] = $payment_details['items'][0]['id'];
            $dataFree['payment_method'] = 'free';

            if(isset($_GET['session_id'])){
                $dataFree['session_id'] = $_GET['session_id'];
            }

            if(isset($_GET['transaction_id'])){
                $dataFree['transaction_id'] = $_GET['transaction_id'];
            }

            $dataFree['amount'] = 0;
            $dataFree['date_added'] = strtotime(date('d M Y'));

            $this->db->insert('bundle_payment', $dataFree);
    ...
    function success_bundle_payment($payment_method = ""){
        ...
        if($model_name != null){
            ...
            $response = $this->$model_name->$payment_check_function($payment_method, 'course_bundle');
        }
